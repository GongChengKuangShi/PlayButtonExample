...
/**
 添加左侧竖线层
 */
//UIBezierPath的详细解释：http://www.jianshu.com/p/2f500c765541
- (void)addLeftLineLayer {
    CGFloat a = self.layer.bounds.size.width;
    
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(a*0.2,a*0.9)];//线条的起始位置
    [path addLineToPoint:CGPointMake(a*0.2,a*0.1)];//线条的终点位置
...
- (void)addLeftCircle {
    ...
    CGFloat startAngle = acos(4.0/5.0) + M_PI_2;//acos:反余弦 结果介于[0, PI]
    CGFloat endAngle = startAngle - M_PI;
    [path addArcWithCenter:CGPointMake(a*0.5, a*0.5)//圆心坐标
                    radius:0.5*a//半径
                startAngle:startAngle//起始的弧度
                  endAngle:endAngle//结束的弧度
                 clockwise:false];//YES为顺时针, NO为逆时针
...
- (void)addRightCircle {
    ...
    CGFloat endAngle = startAngle - M_PI;
    [path addArcWithCenter:CGPointMake(a*0.5, a*0.5)
                    radius:0.5*a
